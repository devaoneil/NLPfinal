...
def main():
    ...
    if args.dataset is not None and (args.dataset.endswith('.json') or args.dataset.endswith('.jsonl')):
        ...
        if args.task == 'qa_adv':
            ...
            print("Loading UCLNLP/adversarial_qa for training")
           ...
           #     print(f"Removing cache at {cache_path}")
            ...
            print("Loading original SQUAD for training")
    ...
    with open('num_examples.csv','w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['count_neg', 'count_non_neg', 'total'])
    train_dataset = None
    eval_dataset = None
