...
def remove_idxs(examples, idx_set):
    return_examples = dict()

    for key, val in examples.items():
        return_examples[key] = [val[i]
                         for i in range(len(examples[key])) if i in idx_set]
    return return_examples
 
def filter_examples_downsample(examples): #down-sample proportionally
    max_examples = 15947  #desired training set size
    total_examples = 30000
    probability_sample = max_examples/total_examples #prob of being sampled
     
    examples["question"] = questions
    count = 0
    i = 0 #index of example
    idx_set = set() #track the ones we want to keep
    while i < len(examples["question"]):  #collect max_examples number of examples randomly
        if random.random() < probability_sample:
            idx_set.add(i) #keep this one
            count += 1  #tracking the kept ones
        i = i + 1
    #keep only those in idx_set
    examples = remove_idxs(examples, idx_set)   
    count_neg = 0
    count_non_neg = 0
    for i in idx_set:
        if " not " in questions[i]:
            count_neg += 1
        else:
            count_non_neg += 1
     #keep track of number of examples so we can keep it consistent
    with open('num_examples_proportional.csv','a', newline='') as f:
        w = csv.writer(f)
        w.writerow([count_neg, count_non_neg, count_neg + count_non_neg])
    return examples

def filter_examples(examples): #filter to remove non-negative questions
    ...
    for i in range(len(questions)):
        if " not " in questions[i]:
            idx_set.add(i) #keep if negative
            count_neg += 1
        elif i % 2 == 0:
            idx_set.add(i) #keep every other one otherwise
            count_non_neg += 1
    #keep track of number of examples so we can keep it consistent
    with open('num_examples.csv','a', newline='') as f:
        w = csv.writer(f)
        w.writerow([count_neg, count_non_neg, count_neg + count_non_neg])
    
    #down-sample questions that don't contain negatives
    #discard every other non-negatively phrased question
    examples = remove_idxs(examples, idx_set)
...
def prepare_train_dataset_qa(examples, tokenizer, max_seq_length=None):
    ...
    #filtered_examples= filter_examples(examples)
    filtered_examples = examples #turn off filtering
    max_seq_length = tokenizer.model_max_length
    tokenized_examples = tokenizer(
        filtered_examples["question"],
        filtered_examples["context"],
